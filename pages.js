...
.btn.small { padding: 6px 10px; background: #eef2ff; color: var(--primary); font-size: 12px }
...
.client-foot { display: flex; justify-content: space-between; align-items: center; color: var(--muted); font-size: 11px; gap: 8px }
.card-actions { display: flex; gap: 4px; flex: 0 0 auto; margin-left: 6px }
.ka-group { display: inline-flex; align-items: center; gap: 8px; flex-wrap: nowrap; min-width: 0 }
.ka { display: inline-flex; align-items: center; gap: 3px; font-size: 11px; color: var(--muted); cursor: pointer; user-select: none; white-space: nowrap }
.ka input { display: none }
.ka i { width: 22px; height: 13px; border-radius: 13px; background: #cdd6e4; position: relative; transition: background .15s; flex: 0 0 auto }
.ka i::after { content: ''; position: absolute; top: 2px; left: 2px; width: 9px; height: 9px; border-radius: 50%; background: #fff; transition: left .15s }
.ka input:checked + i { background: var(--green) }
.ka input:checked + i::after { left: 11px }
.ka-last { font-size: 9.5px; color: var(--muted); white-space: nowrap; margin-left: -4px }
...
function flagImg(c) {
  const emoji = flagFor(c.country || '其他');
  const code = [...emoji]
    .filter((ch) => ch.codePointAt(0) >= 0x1f1e6 && ch.codePointAt(0) <= 0x1f1ff)
    .map((ch) => String.fromCharCode(ch.codePointAt(0) - 0x1f1e6 + 97))
    .join('');
  if (code.length === 2) {
    return `<img class="flag-img" src="https://flagcdn.com/w80/${code}.png" alt="${e(emoji)}" width="40" height="30" loading="lazy" onerror="this.replaceWith('${e(emoji)}')">`;
  }
  return e(emoji);
}

function cardHtml(c, csrf, isGuest) {
  const kaToggles = isGuest ? '' : `<div class="ka-group"><label class="ka" title="按所选保活模式自动访问该网址（离线/定时）"><input type="checkbox" class="ka-cb" data-id="${e(c.id)}"${Number(c.keepalive) === 1 ? ' checked' : ''} onchange="toggleKa(this)"><i></i>保活</label><span class="ka-last" data-id="${e(c.id)}" data-kind="ka"></span><label class="ka" title="按所选继期模式自动访问继期网址"><input type="checkbox" class="rn-cb" data-id="${e(c.id)}"${Number(c.renew) === 1 ? ' checked' : ''} onchange="toggleRn(this)"><i></i>继期</label><span class="ka-last" data-id="${e(c.id)}" data-kind="rn"></span></div>`;
...
function editClient(c) {
  ...
  const kaDisp = iv > 0 && iv % 1440 === 0 ? { v: iv / 1440, u: 'day' } : (iv > 0 && iv % 60 === 0 ? { v: iv / 60, u: 'hour' } : { v: iv, u: 'min' });
  document.getElementById('edit-ka').value = kaDisp.v;
  document.getElementById('edit-ka-unit').value = kaDisp.u;
  document.getElementById('edit-ka-mode1').checked = Number(c.ka_mode || 2) !== 2;
  document.getElementById('edit-ka-mode2').checked = Number(c.ka_mode || 2) === 2;
  const rnIvD = parseInt(c.rn_interval) || 0;
  const rnDisp = rnIvD > 0 && rnIvD % 1440 === 0 ? { v: rnIvD / 1440, u: 'day' } : (rnIvD > 0 && rnIvD % 60 === 0 ? { v: rnIvD / 60, u: 'hour' } : { v: rnIvD, u: 'min' });
  ...
  document.getElementById('edit-rn-mode1').checked = Number(c.renew_mode || 2) === 1;
  document.getElementById('edit-rn-mode2').checked = Number(c.renew_mode || 2) !== 1;
